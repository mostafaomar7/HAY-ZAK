...
import { hijriMonthName, hijriSupported, toHijriParts } from '@core/utils/hijri.utils';
import { controlChanges } from '@core/utils/form-signals';
import { UiButton } from '@shared/components/ui-button/ui-button';
import type { IconName } from '@shared/components/ui-icon/ui-icon';
import { UiHijriDate } from '@shared/components/ui-hijri-date/ui-hijri-date';
...
@Component({
  ...
  imports: [ReactiveFormsModule, RouterLink, UiButton, UiHijriDate, UiIcon, UnitResultCard],
...
export class HomePage {
  ...
  /**
   * The Hijri grid hands back a plain Gregorian date, which goes straight into
   * the same control the native input writes to. That is the whole contract:
   * one value, one calendar on the wire, whichever one the person read.
   */
  protected setHijriDate(plain: string): void {
    this.form.controls.startDate.setValue(plain);
  }
