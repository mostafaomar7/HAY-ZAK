 ...
 * from the payments model.
 *
 * Three, not four: `PROCESSING` was a guess and the backend confirmed it does
 * not exist. `refundStatusText` still falls back to the raw string, so a value
 * nobody predicted shows as itself instead of disappearing.
 */
