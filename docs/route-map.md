...
| `/my-bookings/:id`           | Booking detail, stage trail, address once confirmed         | `permissionGuard([CreateBooking])`  | built |
| `/my-bookings/:id/invoice`   | ZATCA tax invoice (FR-PAY-09)                               | `permissionGuard([CreateBooking])`  | built |
| `/my-bookings/:id/complaint` | Raise a complaint — there is no cancellation                | `permissionGuard([RaiseComplaint])` | built |
